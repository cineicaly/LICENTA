...
from engine.trackers.bytetrack.kalman_filter import KalmanFilter
from engine.trackers.bytetrack import matching
from engine.trackers.bytetrack.basetrack import BaseTrack, TrackState
...
class BYTETracker(object):
    ...
    def update(self, bboxes, confs, classes, _):
        self.frame_id += 1
        activated_starcks = []
        refind_stracks = []
        lost_stracks = []
        ...
        for t in output_stracks:
            ...
            output.extend(np.array(xyxy, dtype=np.int32))
            output.append(tid)
            output.append(int(t.cls))
            output.append(t.score)
            outputs.append(np.array(output, dtype=object))
